        ...
        for hbatch in tqdm(loader):
            ...
            for i in range(len(hbatch)):
                ...
                if L > 0:
                    ...
                    if i == 0:  # Only first sample
                        l, r = hL[i].index(args.cdr_type), hL[i].rindex(args.cdr_type)
                        cdr_len = r - l + 1
                        print(f"\n=== DEBUG RMSD ===")
                        print(f"  CDR range: [{l}, {r}], length: {cdr_len}")
                        print(f"  X_pred shape: {X_pred.shape}")
                        print(f"  X_pred[:,:,1,:] shape: {X_pred[:,:,1,:].shape}")
                        print(f"  hX[i:i+1, l:r+1, 1, :] shape: {hX[i:i+1, l:r+1, 1, :].shape}")
                        print(f"  X_pred first CA: {X_pred[0, 0, 1, :]}")
                        print(f"  True first CA:   {hX[i, l, 1, :]}")
                        print(f"  X_pred last CA:  {X_pred[0, -1, 1, :]}")
                        print(f"  True last CA:    {hX[i, r, 1, :]}")
                    # === END DEBUG ===
