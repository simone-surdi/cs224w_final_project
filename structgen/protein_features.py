...
class CrossDistanceFeatures(nn.Module):
    """
    Compute distance-based features between antigen and antibody.
    
    This captures the spatial relationship between the two structures,
    which is lost when encoding them separately.
    
    Features computed for each antigen residue:
    1. Distance to nearest antibody CA
    2. Distance to K nearest antibody residues (RBF encoded)
    3. Indices of K nearest antibody residues (for cross-attention)
    4. Interface flag (within cutoff distance)
    """
    
    def __init__(self, num_rbf=16, top_k=8, interface_cutoff=10.0):
        """
        Args:
            num_rbf: Number of RBF features for distance encoding
            top_k: Number of nearest antibody neighbors to consider
            interface_cutoff: Distance cutoff for "interface" residues (Angstroms)
        """
        super(CrossDistanceFeatures, self).__init__()
    ...
    def _rbf(self, D):
        ...
        Args:
            D: Distances tensor [..., ] any shape
            
        Returns:
            RBF features [..., num_rbf]
        """
        D_min, D_max = 0., 30.  # Distance range in Angstroms
        D_mu = torch.linspace(D_min, D_max, self.num_rbf).to(D.device)
        D_mu = D_mu.view([1] * len(D.shape) + [-1])  # Broadcast shape
        D_sigma = (D_max - D_min) / self.num_rbf
        D_expand = D.unsqueeze(-1)
        RBF = torch.exp(-((D_expand - D_mu) / D_sigma)**2)
        return RBF
    
    def forward(self, X_ab, X_ag, mask_ab, mask_ag):
        """
        Compute cross-distance features.
        
        Args:
            X_ab: Antibody CA coordinates [B, N, 3]
            X_ag: Antigen CA coordinates [B, M, 3]
            mask_ab: Antibody mask [B, N]
            mask_ag: Antigen mask [B, M]
            
        Returns:
            Dictionary with:
            - 'D_cross': Full distance matrix [B, M, N]
            - 'D_min': Min distance to antibody per antigen residue [B, M]
            - 'D_knn': Distance to K nearest antibody residues [B, M, K]
            - 'E_idx_cross': Indices of K nearest antibody residues [B, M, K]
            - 'RBF_min': RBF encoding of min distance [B, M, num_rbf]
            - 'RBF_knn': RBF encoding of KNN distances [B, M, K, num_rbf]
            - 'interface_mask': Binary mask for interface residues [B, M]
            - 'idx_min': Index of nearest antibody residue [B, M]
        """
        B = X_ab.size(0)
        N = X_ab.size(1)  # Antibody length
        M = X_ag.size(1)  # Antigen length
        K = min(self.top_k, N)
        
        # === COMPUTE FULL CROSS-DISTANCE MATRIX ===
        # D_cross[b, i, j] = distance from antigen residue i to antibody residue j
        dX = X_ag.unsqueeze(2) - X_ab.unsqueeze(1)  # [B, M, N, 3]
        D_cross = torch.sqrt(torch.sum(dX ** 2, dim=-1) + 1e-8)  # [B, M, N]
        
        # Mask invalid positions (set to large distance)
        mask_2D = mask_ag.unsqueeze(2) * mask_ab.unsqueeze(1)  # [B, M, N]
        D_cross_masked = D_cross + (1 - mask_2D) * 1e6
        
        # === MINIMUM DISTANCE (nearest antibody residue) ===
        D_min, idx_min = D_cross_masked.min(dim=-1)  # [B, M]
        D_min = D_min * mask_ag  # Zero out invalid antigen positions
        
        # === K NEAREST NEIGHBORS ===
        D_knn, E_idx_cross = torch.topk(D_cross_masked, K, dim=-1, largest=False)  # [B, M, K]
        
        # === RBF ENCODINGS ===
        RBF_min = self._rbf(D_min)  # [B, M, num_rbf]
        RBF_knn = self._rbf(D_knn)  # [B, M, K, num_rbf]
        
        # === INTERFACE MASK ===
        # Antigen residues within cutoff distance of antibody
        interface_mask = (D_min < self.interface_cutoff).float() * mask_ag  # [B, M]
        
        return {
            'D_cross': D_cross,
            'D_min': D_min,
            'D_knn': D_knn,
            'E_idx_cross': E_idx_cross,
            'RBF_min': RBF_min,
            'RBF_knn': RBF_knn,
